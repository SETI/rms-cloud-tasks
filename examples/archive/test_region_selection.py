...
async def test_region_selection(provider, config_file, specify_region=None, use_spot=False):
    ...
    instance_type = await orchestrator.instance_manager.get_optimal_instance_type(
        cpu_required=2, memory_required_gb=4, disk_required_gb=20, use_spot=use_spot
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--provider", required=True, choices=["aws", "gcp", "azure"], help="Cloud provider to test"
    )
    ...
    # Run the test
    asyncio.run(test_region_selection(args.provider, args.config, args.region, args.use_spot))
