...
@pytest.fixture
def sqs_queue():
    """Fixture to provide an AWSSQSQueue instance with a mock client."""
    queue = AWSSQSQueue()
    return queue


@pytest.mark.asyncio
async def test_initialize(sqs_queue, mock_sqs_client):
    ...
    await sqs_queue.initialize('test-queue', config)
    ...
    assert sqs_queue.queue_url == 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue'


@pytest.mark.asyncio
async def test_initialize_existing_queue(sqs_queue, mock_sqs_client):
    ...
    mock_sqs_client.get_queue_url.return_value = {'QueueUrl': 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue'}
...
@pytest.mark.asyncio
async def test_send_task(sqs_queue, mock_sqs_client):
    """Test sending a task to the queue."""
    # Set up queue
    sqs_queue.queue_url = 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue'
    sqs_queue.sqs = mock_sqs_client
...
@pytest.mark.asyncio
async def test_receive_tasks(sqs_queue, mock_sqs_client):
...
@pytest.mark.asyncio
async def test_complete_task(sqs_queue, mock_sqs_client):
...
@pytest.mark.asyncio
async def test_fail_task(sqs_queue, mock_sqs_client):
    """Test failing a task."""
    # Set up queue
    sqs_queue.queue_url = 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue'
    sqs_queue.sqs = mock_sqs_client
...
@pytest.mark.asyncio
async def test_get_queue_depth(sqs_queue, mock_sqs_client):
...
@pytest.mark.asyncio
async def test_purge_queue(sqs_queue, mock_sqs_client):
